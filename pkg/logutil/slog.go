...
func (s *slogHandler) Handle(r slog.Record) error {
	...
	for _, attr := range s.attrs {
		if attr.Key == "name" {
			_, _ = fmt.Fprintf(withNameColor(attr.Value.String())(buf), " %s", attr.Value.String())
			break
		}
	}
	...
	for _, attr := range s.attrs {
		if attr.Key != "name" {
			...
			case slog.KindLogValuer:
				_, _ = fmt.Fprintf(WithColor(FgWhite)(buf), " %s=%v", attr.Key, attr.Value.LogValuer().LogValue().Any())
			case slog.KindString:
				_, _ = fmt.Fprintf(WithColor(FgWhite)(buf), " %s=%q", attr.Key, attr.Value)
			default:
				_, _ = fmt.Fprintf(WithColor(FgWhite)(buf), " %s=%v", attr.Key, attr.Value)
			}
		}
	}
	_, _ = fmt.Fprint(buf, "\n")
