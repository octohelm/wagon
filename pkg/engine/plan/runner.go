...
func (r *Runner) Run(ctx context.Context, action []string) (*Value, error) {
	...
	if output := ret.LookupPath(r.target).LookupPath(cue.ParsePath("result")); output.Exists() {
		out := output.Value()
		logr.FromContext(ctx).WithValues("result", logutil.CueValue(out)).Info("done")
	}

	if output := ret.LookupPath(r.target).LookupPath(cue.ParsePath("output")); output.Exists() {
		out := output.Value()
		logr.FromContext(ctx).WithValues("output", logutil.CueValue(out)).Info("done")
