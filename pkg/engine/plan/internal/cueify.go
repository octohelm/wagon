...
func (c *convert) toCueType(tpe reflect.Type, naming string) []byte {
	if naming == "" && tpe.PkgPath() != "" {
	...
	switch tpe.Kind() {
	case reflect.Ptr:
		return []byte(fmt.Sprintf("%s | null", c.toCueType(tpe.Elem(), "")))
	case reflect.Map:
		return []byte(fmt.Sprintf("[X=%s]: %s", c.toCueType(tpe.Key(), ""), c.toCueType(tpe.Elem(), "")))
	case reflect.Slice:
		return []byte(fmt.Sprintf("[...%s]", c.toCueType(tpe.Elem(), "")))
	case reflect.Struct:
		b := bytes.NewBuffer(nil)
...
func walkFields(s reflect.Type, each func(info *fieldInfo)) {
	for i := 0; i < s.NumField(); i++ {
		...
		if strings.Contains(wagonTag, "generated") {
			info.name = "output"
		}
